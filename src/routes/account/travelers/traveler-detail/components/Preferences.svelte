...
import { personalPreferenceTypeStore, TravelPreference, TravelPreferenceType, travelPreferenceTypeStore } from '$lib/store/preference';
...
import { get } from 'svelte/store';
    ...
    const preferences: {name: string, value: string}[] = travelPreferenceTypes.reduce((acc , item)=>{
        const selected: TravelPreference[] = [];
        for(let travelPreference of item.preferences){
            const checkExist = traveller.travelPreferences.find(itemPreference=>itemPreference.id === travelPreference.id);
            if(checkExist){
                selected.push(checkExist);
            }
        }
        acc.push({name: item.name,value: selected.reduce((accSelected: string, itemSelected, index)=>{
            if(index > 0){
    ...
    personalPreferenceTypes.map((item)=>{
        ...
        preferences.push({name: item.name,value: selected.reduce((accSelected: string, itemSelected, index)=>{
            if(index > 0){
